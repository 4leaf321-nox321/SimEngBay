...
from app.modules.simulations import routes as simulations_routes
from app.modules.simulations import services as simulations_services
...
def _api_router(settings: Settings) -> APIRouter:
    ...
    # --- 도메인 라우터 ---------------------------------------------------
    router.include_router(simulations_routes.router)
...
def _register_extensions() -> None:
    ...
    # --- 해석 작업 -----------------------------------------------------------
    extensions.register_stats(simulations_services.stats)
    # 실패한 작업이 홈에 뜬다 — 도는 것은 기다리면 되지만 실패한 것은 사람이 봐야 한다.
    extensions.register_maintenance(simulations_services.maintenance)
    extensions.register_workspace_reference(simulations_services.workspace_reference)
    extensions.register_workspace_content(simulations_services.workspace_content)
    # 오케스트레이터가 PAT 로 작업을 건다. **안 열면 기계 자격으로 못 건다** — 그것이 맞는
    # 기본값이다(shared/scopes.py). 재시도도 같은 앞머리라 같은 범위다.
    scopes.register_write_scope("/api/simulations", "simulations:write")
...
def _guard_writable_paths(settings: Settings) -> None:
    ...
    targets = (
        ...
        # 해석 작업 폴더. 기본은 filestore 아래지만 windows-bridge 는 NTFS 쪽을 따로 준다.
        ("WORK_DIR", settings.work_dir or settings.filestore_dir / "simulations"),
    )
