...
SimEngBay 는 StandardPlatform 을 포크한 별도 제품군이다. 그래도 설치마다 `.env` 의
`APP_SLUG` · `APP_NAME` · `APP_TAGLINE` · `EXTENSIONS` 로 이름을 덮을 수 있다(`app/config.py`)
— 개발 · 시험 · 운영이 서로 다른 이름과 DB 를 쓰는 데 그 길을 쓴다.
그래서 여기 있는 이름은 **아무것도 안 준 설치의 기본값**(개발 PC · 시험)일 뿐이다. 코드가
제품 이름을 쓰려면 `get_settings().app_name` 을 읽는다 — 여기서 import 하지 않는다.

## 오류 코드 접두사

`SEB-<MODULE>-<NNNN>`. 이것만은 **빌드에 박힌다** — 코드 곳곳의 `errors.code()` 가 조립하는
...
DEFAULT_APP_SLUG = "simengbay"

#: `.env` 에 `APP_TAGLINE` 이 없을 때 — 한 줄 설명. 로그인 화면과 사이드바가 같은 말을 한다.
DEFAULT_APP_TAGLINE = "Simulation Engineering Bay"
