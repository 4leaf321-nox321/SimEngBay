...
# ── 해석 작업 워커 — 앱과 같은 이미지로 `python -m app.worker`. 없으면 작업이 queued 에 쌓인다. ──
setup_worker() {
    [[ -f "$HERE/worker.service.template" ]] \
        || { warn "worker.service.template 없음 — 워커 유닛 건너뜀"; return 0; }
    info "해석 작업 워커 유닛 렌더 → $WORKER_SERVICE_UNIT (인스턴스 $WORKER_COUNT 개)"
    render_unit_paths "$HERE/worker.service.template" > "$WORKER_SERVICE_UNIT"
    chmod 644 "$WORKER_SERVICE_UNIT"
    systemctl daemon-reload

    # **줄인 수만큼 내린다.** 전에 4개를 띄운 서버에서 WORKER_COUNT=2 로 다시 깔면, 안 내린
    # 둘이 옛 이미지로 계속 돈다 — 그리고 그 사실은 아무 데도 안 적힌다.
    local running
    running="$(systemctl list-units --plain --no-legend "${WORKER_SERVICE_NAME}@*.service" 2>/dev/null | awk '{print $1}')"
    local unit index
    for unit in $running; do
        index="${unit#"${WORKER_SERVICE_NAME}@"}"; index="${index%.service}"
        if [[ "$index" =~ ^[0-9]+$ ]] && (( index > WORKER_COUNT )); then
...
cmd_install() {
    ...
    setup_worker || warn "해석 작업 워커 설정 건너뜀(비치명적)"
...
cmd_update() {
    ...
    # 워커 · MCP 도 함께 갱신(유닛 재렌더 + 재기동). 비치명적.
    # **워커를 빠뜨리면 API 는 새 코드, 워커는 옛 이미지로 돈다** — 같은 SIF 를 쓰므로
    # 재기동만 하면 맞는다.
    setup_worker || warn "해석 작업 워커 설정 건너뜀(비치명적)"
    setup_mcp || warn "MCP 설정 건너뜀(비치명적)"
    setup_sync_timer || warn "동기화 타이머 건너뜀(비치명적)"
...
cmd_setup() {
    ...
    WORKER_SERVICE_NAME="${APP_SLUG}-worker"; WORKER_SERVICE_UNIT="/etc/systemd/system/${WORKER_SERVICE_NAME}@.service"
...
cmd_render() {
    ...
    for tpl in app.service worker.service sync.service backup.service; do
        ...
        case "$tpl" in
            ...
            # 인스턴스 유닛이라 이름에 @ 가 붙는다 — 실제 배포가 남기는 파일 이름과 같게 둔다.
