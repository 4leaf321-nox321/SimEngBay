...
export function visibleGroups(
  ...
  // 확장은 홈 바로 아래, 공통 화면 앞 — 「이 설치만의 것」 이 「어디에나 있는 것」 보다 앞선다.
  const withExtensions =
    extension.length === 0
      ? NAV_GROUPS
      : [NAV_GROUPS[0], ...extension, ...NAV_GROUPS.slice(1)];
...
export function pendingItems(): NavItem[] {
  return NAV_GROUPS.flatMap((group) => group.items).filter(
    (item) => item.pending && item.to,
  );
}
