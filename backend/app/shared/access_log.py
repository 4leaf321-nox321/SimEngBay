...
#: 해석 작업 상태 폴링(`/api/simulations/<id>/status`)이 지금 유일한 것이다.
_SKIP_PATTERNS = (re.compile(r"^/api/simulations/[0-9a-f-]+/status$"),)


def is_skipped(path: str) -> bool:
    """남기지 않을 경로인가.

    지금은 GET 을 아예 안 남기므로(`_RECORDED_METHODS`) 폴링 경로가 GET 이면 여기 없어도
    같은 결과다. 그래도 적어 두는 이유: 폴링이 POST 로 바뀌거나 GET 을 남기기로 하는 날,
    **그 결정을 내리는 사람이 폴링 경로를 일일이 찾아내야 하는** 상태가 안 되게.
    """
    return path in _SKIP or any(one.match(path) for one in _SKIP_PATTERNS)
...
class AccessLogMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if not path.startswith("/api/") or is_skipped(path):
            return
