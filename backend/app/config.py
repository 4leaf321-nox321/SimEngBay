...
class Settings(BaseSettings):
    ...
    StandardPlatform 8040, (예약) PartTrace 8050, AutoJigGenerator 8060, **이 플랫폼 8070**.
    정본 표는 StandardPlatform/docs/새-플랫폼-만들기.md 3.6 이다.
    ...
    **Apptainer 는 호스트 네트워크를 그대로 쓴다**(포트 매핑이 없다). 컨테이너가
    ...
    127.0.0.1 로 한다. 개발 백엔드는 8071 을 쓴다(run.py)."""
    ...
    **운영에서는 반드시 bind-mount 된 경로여야 한다**(`/data/filestore`). SIF 루트는
    ...
    """작업을 워커 없이 **요청 안에서** 돌린다. 시험과 워커 없는 작은 설치가 쓴다. 두 길이 같은
    `services.execute` 를 지나므로 「워커에서는 되는데 인라인에서는 안 되는」 차이가 안
    생긴다."""
    simulation_executor: str = "fake"
    ...
    """fake 실행기가 단계마다 쉬는 시간. 화면의 타임라인이 움직이는 것을 보려면 1~2 초를
    준다."""
    windows_python: Path | None = None
    """`windows-bridge` 가 부를 Windows 파이썬(WSL 에서 보이는 경로).

    개발 PC 의 Ansys 가 Windows 에만 있어서 둔다 — WSL 의 워커가 이 파이썬을 불러 모델링 ·
    솔브 · 추출을 그쪽에서 돌린다. 플로팅 라이선스를 WSL 이 보게 되는 날 `local` 로 바꾸고
    이 값은 지운다(docs/해석-연동-계획.md 3.1)."""
    ansys_version: int = 252
    """Ansys 버전 번호(252 = 2025 R2).

    설치본 경로 · 실행 파일 이름 · 임베디드 App 이 이것을 쓴다."""
    ...
    **워커 수 x 이 값이 기계의 코어를 넘지 않게 한다** — 넘으면 서로를 느리게 만들 뿐이다."""
    ...
    """작업 폴더의 뿌리. 비우면 `filestore_dir/simulations`. **windows-bridge 면 NTFS 쪽**
    (`/mnt/c/simengbay/work`)을 준다 — Ansys 가 `\\wsl$` 에 쓰면 느리고 잠금 문제가 난다."""
    ...
    **서버가 자기 백업을 볼 수 있어야 한다.** 백업은 작업 스케줄러가 돌리는데,
    ...
    cors_origins: list[str] = Field(
        default_factory=lambda: ["http://localhost:5240", "http://127.0.0.1:5240"]
    )
