...
**결과를 추출해 웹에서 보고 오케스트레이터에 돌려주는** 플랫폼.

    오케스트레이터 ──JobSpec──▶ 이 서버
                                 │  CAD 플랫폼에서 STEP/x_t + topology.json
                                 │  물성 DB 에서 물성 → MatML
                                 ▼
                          PyMechanical 워커 ── 형상 임포트 · 물성 · Named Selection · 메시 · BC ──▶ .dat (+ .mechdb)
                                 │
                          솔버(Linux · Slurm, 별도 시스템) ──▶ .rst
                                 │
                          DPF 추출 ── 고유진동수 · 컨투어 · 모드형상 ──▶ JSON · PNG · VTP
                                 │
                          웹 뷰어 · 콜백(오케스트레이터)

**지금은 공통 틀만 있다** — 로그인 · 계정 · 부서 · 권한 · 공지 · 알림 · 감사 · 첨부 · 서버 상태 ·
Apptainer 배포. 해석 도메인은 [docs/로드맵.md](docs/로드맵.md) 의 순서로 얹는다.
...
| **모달 해석** | PyMechanical 임베디드로 임포트 · 물성 · 메시 · `.dat`, MAPDL 이 풀고, DPF 가 고유진동수를 읽는다. **자유-자유**(구속은 4단계) · 실행기 `fake` · `local` · `windows-bridge`. 실측은 [계획서](docs/해석-연동-계획.md) 1.5단계 |
| **결과 보기** | 고유진동수 표(쪽 나누기) · 그림 셋(**스펙트럼** · 누적 유효질량 · 모드별 막대) · 모드 썸네일 · 3D 뷰어(vtk.js, 회전 · 컬러바 · 모드 형상 애니메이션). 뷰어는 결과 화면에서만 받는다 |
| 인증 | 로그인, 세션 회전(refresh httpOnly 쿠키), 강제 비밀번호 변경, 개인 액세스 토큰(PAT) — 오케스트레이터가 이것으로 붙는다 |
