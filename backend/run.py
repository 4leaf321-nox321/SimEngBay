...
## 개발에서는 해석 작업 워커도 함께 띄운다

작업은 큐에 걸리고 워커(`app/worker.py`)가 돌린다. 두 터미널에서 두 명령을 치는 것은 곧 안 하게
되고, 그러면 「작업을 걸었는데 queued 에서 안 움직인다」 를 겪는다. 그래서 개발 모드는 워커를
자식으로 띄우고 이 프로세스가 끝날 때 같이 내린다. `WORKER_DEV=0` 이나 `JOBS_INLINE=1` 이면
안 띄운다. **코드가 바뀌면 워커도 다시 뜬다** — uvicorn 의 reload 는 API 만 새로 띄우므로
watchfiles 로 감싼다(AutoJigGenerator 에서 실측: 옛 워커가 지운 컬럼을 SELECT 하다 매 루프
죽었다).
"""
...
def _start_worker() -> subprocess.Popen[bytes] | None:
    ...
    print(f"해석 작업 워커(watchfiles): pid {child.pid}", file=sys.stderr)
    return child
