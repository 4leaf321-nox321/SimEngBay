...
/**
 * 바이너리를 **화면에 그리려고** 받는다(모드 형상 VTP · 썸네일 PNG).
 *
 * `downloadFile` 과 다른 점은 저장하지 않는다는 것뿐이다 — 둘 다 `<img src>` · `<a href>` 로는
 * 안 되는 같은 이유를 안고 있다(access 토큰이 메모리에만 있어서 브라우저가 스스로 여는 주소에는
 * 안 실린다). 받은 것은 부르는 쪽이 `URL.createObjectURL` 로 쓰고 **반드시 해제한다** —
 * 안 하면 모드를 넘길 때마다 blob 이 쌓인다.
 */
