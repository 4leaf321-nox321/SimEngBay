...
def test_해석_코어는_웹과_DB_를_모른다() -> None:
    """`app/core` 는 순수 파이썬이다 — `fastapi` · `sqlalchemy` · `app.modules` ·
    `app.shared` 를
    import 하지 않는다. 그래야 모델링 스크립트를 서버 없이 돌리고 단위 시험이 DB 없이 코어만
    본다. 모델링 스크립트를 고칠 때마다 DB 를 띄우면 아무도 안 고친다(AGENTS.md).
    """
    forbidden = (
        "fastapi",
        "sqlalchemy",
    ...
    for path in (BACKEND / "app" / "core").rglob("*.py"):
        for name in _imports(path):
            assert not name.startswith(forbidden), (
                f"{path.relative_to(BACKEND)} 이 {name} 을 압니다 — core 는 웹 · DB 를 모른다"
            )
